...
if [ ${PRLL} -ne 1 ]; then
  ...
  sh=$(cat ${INIFILE} | sed '/^#/ d' | grep -i "SkipHistos" | sed 's/[^0-9]//g')
  se=$(cat ${INIFILE} | sed '/^#/ d' | grep -i "SkipEffs" | sed 's/[^0-9]//g')
  if [ -z "${sh}" ]; then
    sh=0
  fi
  if [ -z "${se}" ]; then
  ...
  if [[ -n $(grep -i "SkipEffs" ${INIFILE}) ]]; then 
    sed -i '/SkipEffs/Id' ./tempor.adl # deletes if SkipEffs Exisrs
  fi
  ex -sc '1i|SkipEffs = 1' -cx tempor.adl # Adds SkipEffs 
else
  if [ $Nalgo -gt 1 ]; then
...
if ! [[ "${PRLL}" =~ ^[0-9]+$ ]] || [ ${PRLL} -gt $numcpu ] || [ ${PRLL} -lt 0 ]; then
  ...
  rm -f histoOut-BP_*.root 2>/dev/null
  echo "**********************************************************************************************"
  ...
  multitask(){
    ...
    if [ $lp -eq $((PRLL-1)) ]; then # calls CLA.sh from temp folders
      ...
      ./CLA.sh $datafile $datatype -i tempor.adl -s $((STRT+lp*intrvl)) -e $((intrvl+EVENTS%PRLL)) -v $VERBOSE
    else
      echo ./CLA.sh $datafile $datatype -i ${INIFILE} -s $((STRT+lp*intrvl)) -e $((intrvl)) -v $VERBOSE
      ./CLA.sh $datafile $datatype -i tempor.adl -s $((STRT+lp*intrvl)) -e $((intrvl)) -v $VERBOSE
    fi
  }
  ...
  if [ $? -eq 0 ]; then
    ...
    for ((k = 1 ; k <= ${Nalgo} ; k++)); do
      for ((i = 0 ; i < ${PRLL} ; i++)); do
        hadd -a ../runs/histoOut-BP_${k}.root \
        ../temp_runs${i}/histoOut-BP_${k}.root # merging all root files
      done
    done
    wait
    ...
    if [ $? -eq 0 ]; then
      for ((k = 1 ; k <= ${Nalgo} ; k++)); do
        root -l -q \
        '../analysis_core/FinalEff.C("histoOut-BP_'${k}'.root", "BP_'${k}'", '$sh', '$se')'
      done
