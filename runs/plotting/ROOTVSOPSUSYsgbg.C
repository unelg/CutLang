{
  ...
  TFile fs("histoOut-VSOPSUSY_sg.root");
  TFile fb("histoOut-VSOPSUSY_bg.root");

  // Get the histograms
  // MET and HT histograms from the signal file
  TH1D* hmetsg = (TH1D*)fs.Get("SR/hmet");
  TH1D* hhtsg = (TH1D*)fs.Get("SR/hht");
  // MET and HT histograms from the BG file
  TH1D* hmetbg = (TH1D*)fb.Get("SR/hmet");
  TH1D* hhtbg = (TH1D*)fb.Get("SR/hht");
  ...
  TCanvas cmet("cmet", "cmet", 620, 500);
