...
f = TFile("histoOut-exHistos.root")
...
Those histograms can be accessed with their names.
There is also a 2-dimensional histogram hmetjet1pt.  We will get to that one shortly. 
'''

# Let's get the histograms from the file.  
# Note that one must specify the absolute path of the histogram within the file.
hjet1pt = f.Get("presel/hjet1pt")
hjet2pt = f.Get("presel/hjet2pt")
hjet3pt = f.Get("presel/hjet3pt")
hjet4pt = f.Get("presel/hjet4pt")
...
# The color names do not work in python / jupyter, though they work when running directly with scripts.  
# We therefore used the number codes which are listed in the link, below the color wheel.
...
hjet1pt.SetFillColor(616+1)
hjet2pt.SetFillColor(600)
hjet3pt.SetFillColor(632+1)
hjet4pt.SetFillColor(416+2)
...
l1 = TLegend(0.65, 0.67, 0.88, 0.87)
l1.SetBorderSize(0)
...
hmetjet1pt = f.Get("presel/hmetjet1pt")
