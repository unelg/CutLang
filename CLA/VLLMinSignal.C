...
void VLLMinSignal::GetPhysicsObjects( Long64_t j, AnalysisObjects *a0 )
{
    ...
    */
    //MUONS
    for (unsigned int i=0; i<mu_pt_NOSYS->size(); i++) {
        alv.SetPtEtaPhiM( mu_pt_NOSYS->at(i)*0.001, mu_eta->at(i), mu_phi->at(i), mu_m->at(i)*0.001 ); // all in GeV
        ...
        adbxm->setPdgID(  mu_charge->at(i)*(-13) );
...
#ifdef __DEBUG__
    std::cout << "Muons OK:"<< mu_pt_NOSYS->size()<<std::endl;
#endif

    //ELECTRONS
    for (unsigned int i=0; i<el_pt_NOSYS->size(); i++) {
        alv.SetPtEtaPhiM( el_pt_NOSYS->at(i)*0.001, el_eta->at(i), el_phi->at(i), el_m->at(i)*0.001 ); // all in GeV
        //        std::cout << "ETA:"<<el_eta->at(i) <<std::endl;
        adbxe= new dbxElectron(alv);
        adbxe->setCharge(el_charge->at(i) );
        adbxe->setPdgID(el_charge->at(i)*(-11) );            
        
        //        adbxe->setPtCone(el_pt_NOSYSvarcone20->at(i));// 30 has // this does not exist in vll analysis 20 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        //        adbxe->setEtCone20(el_pt_NOSYSvarcone20->at(i) );
...
#ifdef __DEBUG__
    std::cout << "Electrons OK:"<< el_pt_NOSYS->size() <<std::endl;
#endif

    //JETS
    for (unsigned int i=0; i<jet_pt_NOSYS->size(); i++) {
        alv.SetPtEtaPhiM( jet_pt_NOSYS->at(i)*0.001, jet_eta->at(i), jet_phi->at(i), jet_m->at(i)*0.001 ); // all in GeV
        ...
        adbxj->set_isbtagged_77(bool(jet_FTag_MV2c10_FixedCutBEff_77_NOSYS->at(i))); // ILKINI ALDIM NE ISE YARIYORKI BU
...
#ifdef __DEBUG__
    std::cout << "Jets OK:"<< jet_pt_NOSYS->size() <<std::endl;
#endif
   //MET
    // met.SetMagPhi( met_NOSYS*0.001,  met_phi_NOSYS); //mev-->gev //I am not sure about met_NOSYS. this should be checked.
     met.SetMagPhi( met_NOSYS->at(5)*0.001,  met_phi_NOSYS->at(5)); //mev-->gev //I am not sure about met_NOSYS. this should be checked.
        // std::cout <<"met_NOSYS values: "<<met_NOSYS->at(i)<<endl;
    // std::cout <<"met_NOSYS size: "<<met_NOSYS->size()<<endl;
#ifdef __DEBUG__
   ...
   // anevt.weight_mc=mcEventWeights;
    ...
    anevt.weight_pileup=pileupWeight_NOSYS;
    //std::cout<< "EvtW: " << anevt.weight_mc<<std::endl;
    //std::cout<< "PileW: " << anevt.weight_pileup<<std::endl;
    anevt.weight_jvt=JVTEfficiency_NOSYS;
    //std::cout<< "JVTW: " << anevt.weight_jvt<<std::endl;
    anevt.correction_weight=1;
    anevt.luminosity_weight=36.2; //fb^-1 for 2015,2016 (r9364)
    //anevt.luminosity_weight=44.3; //fb^-1 for 2017 (r10201)
    //anevt.luminosity_weight=58.4; //fb^-1 for 2018  (r10724)
    
    anevt.weight_xsec=222.6; //for 150GeV
    //anevt.weight_xsec=36.9; //for 250GeV
    //cout<<"Xsec: "<<anevt.weight_xsec<<endl;
...
void VLLMinSignal::Loop(analy_struct aselect, char *extname)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   cout << "I am in VLLMinSignal.C " << endl;
   if (fChain == 0) {
